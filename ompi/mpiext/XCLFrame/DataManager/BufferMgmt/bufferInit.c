...
int initNewBuffer(int taskIdx, int trayIdx, int bufferSize ){
	...
	int myRack=l_taskList[taskIdx].Rack;
	...
	if (memIdx==0){//TODO: WARNING!!!!! memindx is cero on rack? <- This is BUG DETECTED
		...
		tmpMemHandler = realloc(l_taskList[taskIdx].device[0].memHandler[myRack],
				(trayIdx + 1) * sizeof(cl_mem)); //(tray + 1) to avoid dereferencing issues.

		if (tmpMemHandler != NULL){
			l_taskList[taskIdx].device[0].memHandler[myRack] = tmpMemHandler;
			...
			free(l_taskList[taskIdx].device[0].memHandler[myRack]);
		...
		/*This section ensures real memory allocation by performing a real writing.*/
...
*/
	...
	for (i = 0; i < numTasks; i++) {
		...
		if (memIdx < trayIdx){ //strictly less to avoid reallocations
			cl_mem * tmpMemHandler;
			tmpMemHandler = realloc(l_taskList[i].device[0].memHandler[myRack],
					(trayIdx+1) * sizeof(cl_mem)); //(tray + 1) to avoid dereferencing issues.
			if (tmpMemHandler != NULL){
				l_taskList[i].device[0].memHandler[myRack] = tmpMemHandler;
				l_taskList[i].numTrays=(trayIdx+1);
			}
			else {
				free(l_taskList[i].device[0].memHandler[myRack]);
		 ...
		 if (trayIdx==0 && memIdx==0){ //if this is the first tray to be initialized.
			...
			l_taskList[i].device[0].memHandler[myRack] = malloc(1 * sizeof(cl_mem)); //init the first device mem space.
		...
		l_taskList[i].device[0].memHandler[myRack][trayIdx] = clCreateBuffer(
